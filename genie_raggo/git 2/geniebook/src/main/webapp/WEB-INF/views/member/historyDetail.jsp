	...
	<div class="container">
						...
						<tbody>
							<c:if test="${fn:length(product)-1 > -1}">
								<c:forEach var="i" begin="0" end="${fn:length(product)-1}"
									...
									<tr>
										<td class="cart_product"><a href="${contextPath}/productDetail?bookNo=${product[i].bookNo}"><c:if
										...
										<td class="cart_description">
											<h4>
												&nbsp;&nbsp;&nbsp;<a href="${contextPath}/productDetail?bookNo=${product[i].bookNo}">${product[i].bookName }</a>
											</h4>
			...
			<table align="center">
				<tr>
					<td>
						<h4>
							<span style="text-align :right; font-size : 25px;">총액</span> <span> <input
								style="border: 0; text-align: right; background: white; font-size : 25px;"
								type="text" id="grandTotal2" value="${history.grandTotal}" readonly>원
							</span>
							<span>(사용한 포인트 : ${history.point })</span>
						</h4>
					</td>
				</tr>
				<tr>
