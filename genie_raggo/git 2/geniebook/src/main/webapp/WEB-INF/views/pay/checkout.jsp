...
<script>
	...
	function payment() {
		/*  if(confirm("구매하시겠습니까?")){
			alert("결제 결과 화면으로 이동합니다.");
			location.href="purchaseHistory?id=${memberVO.id}";
		} else {
			alert("구매가 취소되었습니다.");
		}  */
		...
		<form action="<c:url value="/payment"/>" method="post" >
			...
			<table align="center">
				<tr align="center">
					<td>
						<h4>
	            			<span style="font-size:25px;">&nbsp;&nbsp;&nbsp;&nbsp;총액</span>
								<c:if test="${grandTotal >= 50000}">
									<span><input
								style="border: 0;text-align: right; background: white;"
								type="text" id="grandTotal1" value="${grandTotal}" readonly>원
									</span><br><br>
									<span id="point1"></span>
									<script>
									   a = document.querySelector('#point1');
									   a.innerText = ('구매시 적립가능한 포인트 : ' + ($("#grandTotal1").val() * 0.02).toString() +'P') ;
								   </script>
								</c:if> 
								<input id="point" name="point" type="hidden" value="${point}"/>
								<c:if test="${grandTotal <50000 }"> 
								<span><input
								style="border: 0; text-align: right; background: white;"
								type="text" id="grandTotal2" value="${grandTotal}" readonly>원
								<span style = "font-size : 15px;">(배송비 2500원과 포인트 ${point }P을 적용한 가격)</span>   
								</span><br><br>
								<span id="point2"></span>
							<script>
							   a = document.querySelector('#point2');
							   a.innerText = ('구매시 적립가능한 포인트 : ' + ($("#grandTotal2").val() * 0.02).toString() +'P') ;
						   </script>				
						 </c:if>
						   <script>	
								   var point = parseInt($("#point").val());
							 		 $("#grandTotal2").val((parseInt($("#grandTotal2").val()) + 2500 -point ));
							 		$("#grandTotal1").val((parseInt($("#grandTotal1").val()) - point ));
						   </script>	
						</h4>
