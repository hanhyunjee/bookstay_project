...
/* 		
	...
	var alert=$('.alert');
	$(".point_input").on("change keyup paste", function() {
	  const currentVal = parseInt('$(this).val()');
	  if(currentVal > maxPoint) {
	...
	function plus_cartItem(index ,cartId, bookNo){
		...
		var point = parseInt($("#point_input").val());
		...
		$.ajax({	
			...
			success : function(data){
				...
				$("#grandTotal2").val(grand-point+parseInt(data.price));
				if($("#grandTotal1").val() >= 50000){  /*50000만원 이상 구매시 배송비 무료 */
					$("#qothd").val(0);
				}
			...
			success : function(data){
				...
				if(data.message == "true"){
					...
					if($("#grandTotal1").val() <= 50000){
						$("#qothd").val(2500);
						$("#grandTotal2").val(grand-point-parseInt(data.price)+2500)
					}
				}else{
					alert('최소 수량은 1 입니다');
				}
			...
			<div class="col-sm-9" style="float: none; margin: 0 auto;">
				<div class="total_area">
					<ul>
						...
						<li>회원님의 포인트 잔액 : ${point} P<span>
									<input id="point_max" type="hidden" value="${point}"/>
									 <input id="point_input" name="point" value="0">P 
									<button id="point_use_btn" onclick="useAllPoint()" >모두사용</button>
									<a id="alert" style="color:red; display:none;">소유 포인트를 초과할수 없습니다!</a></span>
						</li>
						<li>배송비<span> <input
								style="border: 0; text-align: right; background: #E6E4DF;"
					...
					<div align="center">
						<a class="btn btn-default update"
							...
							href="<c:url value="/checkout/${id}/${cartId}/${point}"/>" >주문하기</a>
					</div>
				</div>
			</div>
		</div>
