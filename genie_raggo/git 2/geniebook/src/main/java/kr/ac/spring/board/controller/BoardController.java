...
public class BoardController {
	...
	public ModelAndView listBoards(@RequestParam(value="page", required = false, defaultValue="1") int page, //page, range 기본값 1
			@RequestParam(value="range", required = false, defaultValue="1") int range, HttpServletRequest request, HttpServletResponse response, 
			Model model, @ModelAttribute("boardVO") BoardVO boardVO) throws Exception {  // url에서 넘어온 파라미터를 자동으로 바인딩 (boardVO 클래스 내에 있는 속성들과 파라미터들 결합)
